...
//    self.navigationController.navigationBar.backgroundColor = [UIColor clearColor];
    ...
    self.salesMapView.showsUserLocation = YES;
    self.salesMapView.zoomEnabled = YES;
...
-(void)showUserLocation:(CLLocation*)location
{
    ...
    // creates a zoom level for map
    MKCoordinateRegion region = MKCoordinateRegionMake(location.coordinate, MKCoordinateSpanMake(.2, .2));
    [self.salesMapView setRegion:region animated:YES];
}

-(void)populatePinsOnMapForEvents
{
    [self.salesMapView removeAnnotations:self.salesMapView.annotations];
...
//        CLGeocoder * geoCoder = [[CLGeocoder alloc] init];
        ...
        [self.salesMapView addAnnotation: annotation];
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  //  cell.sellerNameLabel.text = seller[@"username"];
   // cell.priceLabel.text = sellingOption[@"PriceForTicket"];
    ...
    cell.numberOfTicketsLabel.text = [NSString stringWithFormat:@"%@ Tickets", sellingOption[@"NumberOfTicketsSelling"]];
